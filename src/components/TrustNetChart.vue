...
const updateHoverDataFromTime = (time: Time | null) => {
  ...
  hoverValue.value = data?.trust ?? null;

  if (chart && series && data?.trust != null) {
    const timeCoord = chart.timeScale().timeToCoordinate(time);
    const priceCoord = series.priceToCoordinate(data.trust);
...
const updateData = () => {
  ...
  const data = store.institutionalData.map((d) => ({
    time: d.time, value: d.trust, color: d.trust >= 0 ? '#f97316' : '#06b6d4'
  }));
...
<template>
  <div class="relative w-full h-full overflow-hidden">
    ...
    <div class="absolute top-1 left-1 z-10 flex items-center gap-1 text-[10px] pointer-events-none">
      <span class="text-white font-bold bg-[#1a1a1a]/80 px-1 rounded">投信買賣超</span>
      <span v-if="hoverValue !== null" :class="hoverValue >= 0 ? 'text-[#f97316]' : 'text-[#06b6d4]'" class="bg-[#1a1a1a]/80 px-1 rounded">{{ formatValue(hoverValue) }}</span>
    </div>
