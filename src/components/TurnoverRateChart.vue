...
const updateHoverDataFromTime = (time: Time | null) => {
  ...
  const data = store.turnoverRateData.find(d => d.time === time);
  hoverValue.value = data?.value ?? null;

  if (chart && series && data?.value != null) {
    const timeCoord = chart.timeScale().timeToCoordinate(time);
    const priceCoord = series.priceToCoordinate(data.value);
...
const initChart = () => {
  ...
  chart = createChart(chartContainer.value, {
    ...
    timeScale: { borderColor: '#333', visible: false, barSpacing: 12, minBarSpacing: 4, rightOffset: 1.5, fixLeftEdge: false, fixRightEdge: false }
  });

  series = chart.addSeries(HistogramSeries, { priceLineVisible: false, lastValueVisible: false, color: '#8b5cf6' });
...
const updateData = () => {
  if (!series) return;
  const data = store.turnoverRateData.map((d) => ({ time: d.time, value: d.value, color: '#8b5cf6' }));
  series.setData(data as any);
};

watch(() => store.turnoverRateData, () => updateData(), { deep: true });
onMounted(() => initChart());
...
<template>
  <div class="relative w-full h-full overflow-hidden">
    ...
    <div class="absolute top-1 left-1 z-10 flex items-center gap-1 text-[10px] pointer-events-none">
      <span class="text-white font-bold bg-[#1a1a1a]/80 px-1 rounded">週轉率</span>
      <span v-if="hoverValue !== null" class="text-[#8b5cf6] bg-[#1a1a1a]/80 px-1 rounded">{{ hoverValue?.toFixed(2) }}%</span>
    </div>
    ...
    <div
      ...
      :style="tooltipStyle"
    >
      <span class="text-[#8b5cf6]">{{ hoverValue?.toFixed(2) }}%</span>
    </div>
  </div>
</template>
